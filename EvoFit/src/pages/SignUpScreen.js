...
function SignUpScreen({ setUser }) {
    ...
    const handleLogin = async () => {
        try {
            ...
            navigation.navigate('Days', { userId: user.uid, nome: username }); // Passa o ID do usuário e o nome cadastrado como parâmetros
        } catch (error) {
    ...
    const addUserToFirestore = async (userId) => {
        const userDocRef = doc(db, 'users', userId); // Define o documento com o ID do usuário
        const userData = {
            userId: userId, // Adiciona o ID do usuário ao documento
            username, 
            email,
            dataNascimento,
            sexo,
            // Verifica se os campos opcionais têm valores antes de adicioná-los ao documento
            ...(altura && { altura }),
            ...(peso && { peso }),
            ...(objetivo && { objetivo }),
        ...
        await setDoc(userDocRef, userData); // Usa setDoc para definir os dados do usuário no documento

        console.log('Usuário adicionado ao Firestore:', userData);
    };
